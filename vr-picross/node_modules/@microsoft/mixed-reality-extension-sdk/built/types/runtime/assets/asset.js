...
class Asset {
    ...
    static Parse(container, def) {
        if (def.prefab) {
            return new _1.Prefab(container, def);
        }
        else if (def.mesh) {
            return new _1.Mesh(container, def);
        }
        else if (def.material) {
            return new _1.Material(container, def);
        }
        else if (def.texture) {
            return new _1.Texture(container, def);
        }
        else if (def.sound) {
            return new _1.Sound(container, def);
        }
        else if (def.videoStream) {
            return new _1.VideoStream(container, def);
        }
        else {
